...
namespace NoktaBilgiNotificationUI.Business
{
   internal class EMailSettingLogic
    {
        internal static bool EmailControl(TextEdit senderEmail, TextEdit senderPassword, TextEdit server, TextEdit port)
        {
            ...
            {
                XtraMessageBox.Show("Port Kutusu 5 Haneden Fazla Olamaz", "Hatalı Port Girişi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                port.Focus();
                return false;
            }
            else if (port.Text.Length > 5)
